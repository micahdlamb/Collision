...
void main(void)
{
	...
	while (alpha < 1 && mover.x >= 0 && mover.x <= 1 && mover.y >= 0 && mover.y <= 1 && mover.z >= 0 && mover.z <= 1){
		...
		float change1 = abs(density - lastAccum);
		float change2 = abs(density - prev);

		if (change1 > threshold){
			float a = density * absorbtion;
			
			vec3 norm = normalize(texture(normals,mover).rgb);

...
struct Ballz : public Viewport, public Scene, public FPInput {
	...
	struct Lenses : public UniformBlock {
		struct Lens {
			vec3 center;
			float radius;
			float ri;
		...
		Lenses():numLenses(0),zpos(0){
			UniformBlock::operator()(sizeof(Lenses)-sizeof(UniformBlock),11);
		}
		...
		float zpos;
		void add(float radius, float sep, float ri, float aperture){
			zpos += sep;
			if (radius == 0) return;
			if (ri == 0) ri = 1;
			auto& lens = lenses[numLenses];
			lens.center = vec3(0,0,-(zpos + -radius));
			lens.radius = std::abs(radius);
			lens.ri = ri;
			lens.aperture = aperture;
			lens.front = radius < 0 ? 1 : 0;
			numLenses++;
		}
		...
		void loadFile(string file){
			...
			for (int i=r.size()-1; i >= 0; i--){
				add(r[i].x, r[i].y, i>0 ? r[i-1].z : 1, r[i].w);
	...
	Ballz(float x, float y, float w, float h):
		Viewport(x,y,w,h)
		,FPInput(10)
		,nearPlane(.1f), farPlane(1000.f), fovY(30.f)
		,dragIndex(-1)
	{
		printGLErrors("init");
		FPInput::operator()(vec3(0,0,1));
		Scene::operator()(this);
		shader("rayTracer.vert", "lens.frag");
		shader.enable();
		shader.addUniformBlock("Spheres", 10);
		shader.addUniformBlock("Lenses", 11);

		lenses.loadFile("fisheye.10mm.dat");
		lenses.sync();
		printGLErrors("/lens sync");
